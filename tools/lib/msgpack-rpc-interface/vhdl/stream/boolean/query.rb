module MsgPack_RPC_Interface::VHDL::Stream::Boolean::Query
  ...
  def generate_stmt(indent, name, data_type, kvmap, registory)
    ...
    if kvmap == true then
      ...
      vhdl_lines = string_to_lines(
        indent, <<"        EOT"
          #{instance_name} : MsgPack_KVMap_Query_Boolean_Stream   -- 
              ...
              port map (                 #{sprintf("%-28s", ""                      )}   -- 
                  ...
                  STRB                => #{sprintf("%-28s", interface[:strb        ])} , -- In  :
                  LAST                => #{sprintf("%-28s", interface[:last        ])} , -- In  :
      ...
      vhdl_lines = string_to_lines(
        indent, <<"        EOT"
          #{instance_name} : MsgPack_Object_Query_Boolean_Stream   -- 
              generic map (              #{sprintf("%-28s", ""                      )}   -- 
                  CODE_WIDTH          => #{sprintf("%-28s", registory[:code_width  ])} , --
                  DATA_BITS           => #{sprintf("%-28s", interface[:data_bits   ])} , --
                  SIZE_BITS           => #{sprintf("%-28s", interface[:size_bits   ])}   --
  ...
  def use_package_list(kvmap)
    if kvmap == true then
      return ["MsgPack.MsgPack_KVMap_Components.MsgPack_KVMap_Query_Boolean_Stream"]
    else
      return ["MsgPack.MsgPack_Object_Components.MsgPack_Object_Query_Boolean_Stream"]
    end
  end
