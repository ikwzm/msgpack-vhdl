module MsgPack_RPC_Interface::VHDL::Memory::Arbitor
  ...
  def generate_decl(indent, name, registory)
    ...
    return string_to_lines(indent, <<"      EOT"
        ...
        signal    #{sprintf("%-17s",registory[:store_valid])} :  std_logic;
        ...
        signal    #{sprintf("%-17s",registory[:query_ready])} :  std_logic;
  ...
  def generate_stmt(indent, name, registory)
    ...
    vhdl_lines << indent + "    #{registory[:oe  ]} <= '1' when (proc_arb_state(0) = '0') else '0';" if registory.key?(:oe)
    vhdl_lines << indent + "    #{registory[:we  ]} <= #{registory[:store_valid]} when (proc_arb_state(0) = '1') else '0';" if registory.key?(:we)
    vhdl_lines << indent + "    #{registory[:addr]} <= #{registory[:store_addr ]} when (proc_arb_state(0) = '1') else #{registory[:query_addr]};" if registory.key?(:addr)
    vhdl_lines.concat(string_to_lines(indent, <<"      EOT"
        end block;
      EOT
    ))
