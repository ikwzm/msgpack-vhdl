module MsgPack_RPC_Interface::VHDL::Register::Integer::Store
  ...
  def generate_stmt(indent, name, type, kvmap, registory)
    ...
    if kvmap == true then
      ...
      vhdl_lines    = string_to_lines(
        indent, <<"        EOT"
          #{instance_name} : MsgPack_Object_Store_Integer_Register                      -- 
              generic map (              #{sprintf("%-28s", ""                     )}   -- 
                  CODE_WIDTH          => #{sprintf("%-28s", registory[:code_width ])} , --
                  VALUE_BITS          => #{sprintf("%-28s", value_bits             )} , --
                  VALUE_SIGN          => #{sprintf("%-28s", value_sign             )} , --
  ...
  def use_package_list(kvmap)
    if kvmap == true then
      ...
      return ["MsgPack.MsgPack_Object_Components.MsgPack_Object_Store_Integer_Register"]
    end
  end
