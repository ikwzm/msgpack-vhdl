module MsgPack_RPC_Interface::VHDL::Register::Boolean::Store
  ...
  def generate_stmt(indent, name, data_type, kvmap, registory)
    ...
    if kvmap == true then
      ...
      vhdl_lines = string_to_lines(
        indent, <<"        EOT"
          #{instance_name} : MsgPack_KVMap_Store_Boolean_Register   -- 
              ...
              port map (                 #{sprintf("%-28s", ""                     )}   -- 
                  ...
                  VALUE               => #{sprintf("%-28s", value_name             )} , -- Out :
      ...
      vhdl_lines    = string_to_lines(
        indent, <<"        EOT"
          #{instance_name} : MsgPack_Object_Store_Boolean_Register                      -- 
              generic map (              #{sprintf("%-28s", ""                     )}   -- 
                  CODE_WIDTH          => #{sprintf("%-28s", registory[:code_width ])}   --
              )                          #{sprintf("%-28s", ""                     )}   -- 
              port map (                 #{sprintf("%-28s", ""                     )}   -- 
  ...
  def use_package_list(kvmap)
    if kvmap == true then
      return ["MsgPack.MsgPack_KVMap_Components.MsgPack_KVMap_Store_Boolean_Register"]
    else
      return ["MsgPack.MsgPack_Object_Components.MsgPack_Object_Store_Boolean_Register"]
    end
  end
