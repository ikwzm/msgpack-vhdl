      ...
      indent, <<"      EOT"
          PROC_MAIN: MsgPack_RPC_Method_Main_No_Param         -- 
              ...
              port map (                     #{sprintf("%-28s", ""                     )}   -- 
                  ...
                  RUN_REQ_VAL             => #{sprintf("%-28s", run_signals[:req_valid])} , -- Out :
                  RUN_REQ_RDY             => #{sprintf("%-28s", run_signals[:req_ready])} , -- In  :
                  RUN_RES_VAL             => #{sprintf("%-28s", run_signals[:res_valid])} , -- In  :
                  RUN_RES_RDY             => #{sprintf("%-28s", run_signals[:res_ready])} , -- Out :
                  RUNNING                 => #{sprintf("%-28s", run_signals[:running  ])} , -- Out :
