module MsgPack_RPC_Interface::VHDL::Procedure::QueryVariables
  ...
  def generate_decl(indent, name, variables, registory)
    ...
    vhdl_lines  = string_to_lines(
      indent, <<"        EOT"
          constant  PROC_MAP_QUERY_SIZE   :  integer := #{variables.size};
  ...
  def generate_stmt(indent, name, variables, registory)
      ...
      indent, <<"        EOT"
          PROC_MAIN: MsgPack_RPC_Server_KVMap_Get_Value         -- 
              generic map (                  #{sprintf("%-28s", ""                     )}   -- 
                  ...
                  STORE_SIZE              => #{sprintf("%-28s", "PROC_MAP_QUERY_SIZE"  )} , --
              ...
              port map (                     #{sprintf("%-28s", ""                     )}   -- 
                  ...
                  MAP_PARAM_START         => #{sprintf("%-28s", "proc_map_param_start" )} , -- Out :
                  MAP_PARAM_VALID         => #{sprintf("%-28s", "proc_map_param_valid" )} , -- Out :
                  MAP_PARAM_CODE          => #{sprintf("%-28s", "proc_map_param_code"  )} , -- Out :
                  MAP_PARAM_LAST          => #{sprintf("%-28s", "proc_map_param_last"  )} , -- Out :
                  MAP_PARAM_ERROR         => #{sprintf("%-28s", "proc_map_param_error" )} , -- In  :
                  MAP_PARAM_DONE          => #{sprintf("%-28s", "proc_map_param_done"  )} , -- In  :
                  MAP_PARAM_SHIFT         => #{sprintf("%-28s", "proc_map_param_shift" )} , -- In  :
                  MAP_VALUE_VALID         => #{sprintf("%-28s", "proc_map_value_valid" )} , -- In  :
                  MAP_VALUE_CODE          => #{sprintf("%-28s", "proc_map_value_code"  )} , -- In  :
                  MAP_VALUE_LAST          => #{sprintf("%-28s", "proc_map_value_last"  )} , -- In  :
                  MAP_VALUE_ERROR         => #{sprintf("%-28s", "proc_map_value_error" )} , -- In  :
                  MAP_VALUE_READY         => #{sprintf("%-28s", "proc_map_value_ready" )} , -- Out :
    ...
    variables.to_a.each_with_index do |variable, num|
      ...
      var_regs[:value_code  ] = "proc_map_value_code (#{num})"
      var_regs[:value_last  ] = "proc_map_value_last (#{num})"
      var_regs[:value_valid ] = "proc_map_value_valid(#{num})"
      var_regs[:value_error ] = "proc_map_value_error(#{num})"
      var_regs[:value_ready ] = "proc_map_value_ready(#{num})"
      ...
      vhdl_lines.concat(variable.interface.generate_vhdl_body_query(indent, var_regs))
    end
    return vhdl_lines
  end
  ...
  def generate_body(indent, name, variable, registory)
    ...
    if (decl_code.size > 0) then
      block_start = registory.fetch(:block_start, "PROC_QUERY_VARIABLES: block")
  ...
  def use_package_list
    return ["MsgPack.MsgPack_RPC_Components.MsgPack_RPC_Server_KVMap_Get_Value"]
  end
